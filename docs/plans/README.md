# Metis Detailed Plans

Per-stage detailed plans. Each refines the corresponding stage in the
[high-level implementation plan](high-level-implementation-plan.md) with an
objective, package ownership, concrete files/modules, schemas/interfaces touched,
implementation steps, tests/fixtures, acceptance criteria, and risks.

| Stage | Plan | Status |
|---|---|---|
| 0 | [Repository and architecture guardrails](stage-00-repo-guardrails.md) | Implemented |
| 1 | [`metis-protocol`](stage-01-protocol.md) | Implemented |
| 2 | [`metis-core`](stage-02-core.md) | Planned |
| 3 | [Local-first ingestion pipeline](stage-03-ingestion.md) | Planned |
| 4 | [Model router and extraction quality loop](stage-04-model-router.md) | Planned |
| 5 | [Memory core](stage-05-memory-core.md) | Planned |
| 6 | [Maintainer worker](stage-06-maintainer.md) | Planned |
| 7 | [Wiki compiler and human-facing knowledge](stage-07-wiki-compiler.md) | Planned |
| 8 | [Retrieval and query runtime](stage-08-retrieval-runtime.md) | Planned |
| 9 | [Skill runtime](stage-09-skill-runtime.md) | Planned |
| 10 | [Runtime-agent integration](stage-10-agent-integration.md) | Planned |
| 11 | [External connectors](stage-11-connectors.md) | Planned |
| 12 | [API, UI, and ops surfaces](stage-12-api-ui-ops.md) | Planned |
| 13 | [Evaluation harness](stage-13-evaluation.md) | Planned |
| 14 | [Security, privacy, and hardening](stage-14-security-hardening.md) | Planned |
| 15 | [Deployment and operational readiness](stage-15-deployment.md) | Planned |
